...
class SampleMemo extends StatefulWidget {
  const SampleMemo({super.key});

  @override
  State<SampleMemo> createState() => _SampleMemoState();
}

class _SampleMemoState extends State<SampleMemo> {
  ...
  final _sentAddressController = TextEditingController(); // almtKirim
  final _sentToController = TextEditingController(); // penerima
  final _sentPhoneController = TextEditingController(); // telp
  List<String> selectedProduct = []; // descBrg
  ...
  void isEditMemo(MemoSample? memo) {
  ...
  void _saveMemo(BuildContext context) async {
    var data = MemoSample(
      ...
      almtKirim: _sentAddressController.text,
      penerima: _sentToController.text,
      telp: _sentPhoneController.text,
      descBrg: selectedProduct.join(','),
      userid: selectedReq,
      ndm: selectedChecker,
    ...
    api.saveMemo<MemoSample>(data, url).then((response) {
      Navigator.pop(context);
      if (response == null) {
  ...
  void dispose() {
    ...
    _sentAddressController.dispose();
    _sentToController.dispose();
    _sentPhoneController.dispose();
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                const Text(
                  'Sehubungan dengan keperluan pengenalan produk PT. UnichemCandi Indonesia ke Customer, maka diajukan sample sebagai berikut :',
                ),
                const SizedBox(height: 16),
                const LabelText(label: 'PIHAK PENERIMA'),
                const SizedBox(height: 16),
                ...
                AppTextField(
                  readOnly: action != MemoAct.create,
                  controller: _sentAddressController,
                  validator: (value) {
                    if (value == null || value.isEmpty) {
                      return 'Alamat kirim tidak boleh kosong';
                    }
                    return null;
                  },
                ),
                ...
                const SizedBox(height: 16),
                const LabelText(label: 'No. Telepon Penerima'),
                AppTextField(
                  readOnly: action != MemoAct.create,
                  controller: _sentPhoneController,
                  validator: (value) {
                    if (value == null || value.isEmpty) {
                      return 'No. telepon penerima tidak boleh kosong';
                    }
                    return null;
                  },
                ),
                const SizedBox(height: 16),
                const LabelText(label: 'Produk sample yang diberikan'),
                DropdownSelectableList<Product>(
                  ...
                  selectedItems: custprod?.products
                      .where((element) => selectedProduct.contains(element.id))
                      .toList(),
                ),
                const SizedBox(height: 16),
                const Text(
                  'Demikian Memo ini diajukan agar dapat digunakan sebagaimana mestinya.\nAtas perhatian dan kerjasamanya kami sampaikan terima kasih.',
