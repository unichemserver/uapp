...
class BarangMemo extends StatefulWidget {
  const BarangMemo({super.key, this.memo});

  final MemoBarangRusak? memo;

  @override
  State<BarangMemo> createState() => _BarangMemoState();
}

class _BarangMemoState extends State<BarangMemo> {
  ...
  final _fromController = TextEditingController(); // frm
  final _subjectProductController = TextEditingController(); // subject
  final _fileNoController = TextEditingController(); // file_no
  final _dateProductController = TextEditingController(); // tgl_b
  final _sheetNoController = TextEditingController(); // sheet_no
  ...
  void _saveMemo(BuildContext context) async {
    MemoBarangRusak data = MemoBarangRusak(
      ...
      frm: _fromController.text,
      subject: _subjectProductController.text,
      fileNo: _fileNoController.text,
      tglB: _dateProductController.text,
      sheetNo: _sheetNoController.text,
      pesan: mechanism,
      userid: selectedReq,
      ndm: selectedChecker,
    ...
    api.saveMemo<MemoBarangRusak>(data, url).then((response) {
      Navigator.pop(context);
      if (response == null) {
  ...
  void isEditMemo(MemoBarangRusak? memo) {
    if (memo != null) {
      isEdit = true;
      this.memo = memo;
      ...
      _fromController.text = memo.frm;
      _subjectProductController.text = memo.subject;
      _fileNoController.text = memo.fileNo;
      _dateProductController.text = memo.tglB;
      _sheetNoController.text = memo.sheetNo;
      status = memo.status;
      ...
      selectedContacts = contacts
          .where((element) => selectedRecipient.contains(element.id))
          .toList();
      setState(() {});
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      Future.wait(<Future<void>>[
        ...
        if (isEdit) {
          selectedTo = custprod?.customers.firstWhereOrNull(
  ...
  void dispose() {
    ...
    _subjectProductController.dispose();
    _fileNoController.dispose();
    _dateProductController.dispose();
    _sheetNoController.dispose();
    _itemKeys.clear();
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                const LabelText(label: 'Subject'),
                AppTextField(
                  readOnly: action != MemoAct.create,
                  controller: _subjectProductController,
                  validator: (value) {
                    if (value == null || value.isEmpty) {
                ...
                const SizedBox(height: 16),
                const LabelText(label: 'File No'),
                AppTextField(
                  readOnly: action != MemoAct.create,
                  controller: _fileNoController,
                  validator: (value) {
                    if (value == null || value.isEmpty) {
                      return 'File No tidak boleh kosong';
                    }
                    return null;
                  },
                ),
                ...
                AppTextField(
                  controller: _dateProductController,
                  readOnly: true,
                ...
                const SizedBox(height: 16),
                const LabelText(label: 'Sheet No.'),
                AppTextField(
                  readOnly: action != MemoAct.create,
                  controller: _sheetNoController,
                  validator: (value) {
                    if (value == null || value.isEmpty) {
                      return 'Sheet No tidak boleh kosong';
                    }
                    return null;
                  },
                ),
                ...
                ElevatedButton(
                  ...
                  child: Text(isEdit ? 'Edit Memo' : 'Simpan Memo'),
                ),
              ],
            ),
          ),
